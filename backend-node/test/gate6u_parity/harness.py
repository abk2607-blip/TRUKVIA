"""Phase 3 · Gate 6u · live Python↔Node parity harness for MechanicWorkOrder reads.
...
STRICT UAT-DATA PRESERVATION:
  Uses an isolated timestamped DB (`trukvia_gate6u_parity_<ts>`) that
...
Class-C stance:
  ...
  `db.mechanic_work_orders.find(...).sort().to_list(5000)` and
  `.find_one(...)`. Zero writer hook / audit / backfill / recompute /
  FinTxn / approvals / policy / counters / idempotency / cross-collection
  reads on GET paths.

Gate-6u dimensions bound in this harness:
  * Soft-delete predicate `is_deleted: {$ne: True}` on both endpoints.
  * `activeCompanyId()` consumed (X-Company-Id changes rowset).
  * 5000 cap on LIST, work_date DESC ordering.
  * DETAIL 404 literal `"MechanicWorkOrder not found"` on miss /
    wrong-company / soft-deleted wid.
"""
...
DB = f"trukvia_gate6u_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8196, 8197
...
def _wo(**kw: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
        "id": "wo-x", "user_id": "u1", "company_id": "co-a",
        "mechanic_id": "mec-1", "mechanic_name": "Alpha Garage",
        "work_date": D1, "amount": 1000.0, "hours_worked": 4.0,
        "vehicle_id": "veh-1", "vehicle_number": "MH01AA0001",
        "trip_id": "trp-1", "repair_event_id": "re-1",
        "narration": "body work", "remarks": "",
        "file_ids": [],
...
def fixtures() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        "mechanic_work_orders": [
            _wo(id="w1", work_date=D1),
            _wo(id="w2", work_date=D2, mechanic_id="mec-2", vehicle_id="veh-2",
                repair_event_id="re-2", trip_id="trp-2"),
            _wo(id="w-sd", work_date=D1, is_deleted=True),
            _wo(id="w-alt", company_id="co-a-alt", work_date=D1),
            _wo(id="w-u2", user_id="u2", company_id="co-b", work_date=D1),
        ],
    }
...
TRACKED = ("users", "companies", "customers", "trips",
           ...
           "repair_events", "mechanic_work_orders")
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6u_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6u_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    {"n":  1, "d": "list happy · u1 · co-a · [w1,w2] DESC (soft-deleted excluded, alt-co hidden)",
     "url": "/api/mechanic-work-orders", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  2, "d": "empty · vehicle_id=no-such → 200 []",
     "url": "/api/mechanic-work-orders?vehicle_id=no-such", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  3, "d": "mechanic_id filter · mec-2 → [w2]",
     "url": "/api/mechanic-work-orders?mechanic_id=mec-2", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  4, "d": "vehicle_id filter · veh-2 → [w2]",
     "url": "/api/mechanic-work-orders?vehicle_id=veh-2", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  5, "d": "repair_event_id filter · re-2 → [w2]",
     "url": "/api/mechanic-work-orders?repair_event_id=re-2", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  6, "d": "trip_id filter · trp-2 → [w2]",
     "url": "/api/mechanic-work-orders?trip_id=trp-2", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  7, "d": "combined filter · mechanic_id + vehicle_id + repair_event_id + trip_id → [w2]",
     "url": "/api/mechanic-work-orders?mechanic_id=mec-2&vehicle_id=veh-2&repair_event_id=re-2&trip_id=trp-2",
     "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  8, "d": "empty-string filter · truthy-gate omission · returns all co-a active rows",
     "url": "/api/mechanic-work-orders?mechanic_id=&vehicle_id=&repair_event_id=&trip_id=",
     "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  9, "d": "owned X-Company-Id override · co-a-alt → [w-alt]",
     "url": "/api/mechanic-work-orders", "hk": "u1",
    ...
    {"n": 11, "d": "cross-user · u2 → [w-u2]",
     "url": "/api/mechanic-work-orders", "hk": "u2", "expect": 200, "compare": "full"},
    {"n": 12, "d": "detail happy · w1",
     "url": "/api/mechanic-work-orders/w1", "hk": "u1", "expect": 200, "compare": "full"},
    {"n": 13, "d": "detail unknown wid → 404 exact literal",
     "url": "/api/mechanic-work-orders/no-such", "hk": "u1", "expect": 404, "compare": "full"},
    {"n": 14, "d": "detail wrong-company · w-u2 belongs to co-b → same 404",
     "url": "/api/mechanic-work-orders/w-u2", "hk": "u1", "expect": 404, "compare": "full"},
    {"n": 15, "d": "detail soft-deleted · w-sd is_deleted:true → same 404",
     "url": "/api/mechanic-work-orders/w-sd", "hk": "u1", "expect": 404, "compare": "full"},
    {"n": 16, "d": "no auth → 401 Not authenticated",
     "url": "/api/mechanic-work-orders", "hk": "none", "expect": 401, "compare": "full"},
    {"n": 17, "d": "invalid bearer → 401 Invalid session",
     "url": "/api/mechanic-work-orders", "hk": "bad", "expect": 401, "compare": "full"},
]
...
async def run() -> int:
    print(f"[gate6u] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate6u] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate6u_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate6u_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 6u · LIVE PARITY MATRIX (MechanicWorkOrders)")
        print("=" * 72)
        for r in results:
