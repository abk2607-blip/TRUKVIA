"""Phase 3 · Gate 7c · live Python↔Node parity harness for
WalletRecharges list reads.

Covers:
  * GET /api/wallet-recharges

STRICT UAT-DATA PRESERVATION:
  Isolated timestamped DB `trukvia_gate7c_parity_<ts>`, dropped in
...
for the collection name (`wallet_recharges`) and route path.
"""
...
DB = f"trukvia_gate7c_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8212, 8213
...
def fixtures() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        "wallet_recharges": [
            _wr(id="r1", wallet_code="WALLET_FUEL",   date=D1),
            _wr(id="r2", wallet_code="WALLET_FASTAG", date=D2),
            _wr(id="r3", wallet_code="WALLET_FUEL",   date=D3),
            _wr(id="r4", wallet_code="WALLET_FUEL",   date=D4),
            _wr(id="r-del", wallet_code="WALLET_FUEL", date=D1, is_deleted=True),
            _wr(id="r-alt", wallet_code="WALLET_FUEL", company_id="co-a-alt", date=D1),
            _wr(id="r-u2",  wallet_code="WALLET_FUEL", user_id="u2", company_id="co-b", date=D1),
        ],
    }
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate7c_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate7c_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    {"n":  1, "d": "default · u1 · co-a · four active rows DESC",
     "url": "/api/wallet-recharges", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  2, "d": "include_deleted=true · deleted included",
     "url": "/api/wallet-recharges?include_deleted=true", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  3, "d": "include_deleted=false · deleted excluded",
     "url": "/api/wallet-recharges?include_deleted=false", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  4, "d": "include_deleted=TRUE",
     "url": "/api/wallet-recharges?include_deleted=TRUE", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  5, "d": "include_deleted=1",
     "url": "/api/wallet-recharges?include_deleted=1", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  6, "d": "include_deleted=yes",
     "url": "/api/wallet-recharges?include_deleted=yes", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  7, "d": "include_deleted=off",
     "url": "/api/wallet-recharges?include_deleted=off", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  8, "d": "include_deleted='' → 422 bool_parsing envelope",
     "url": "/api/wallet-recharges?include_deleted=", "hk": "u1", "expect": 422, "compare": "full"},
    {"n":  9, "d": "include_deleted=FOO → 422 bool_parsing envelope",
     "url": "/api/wallet-recharges?include_deleted=FOO", "hk": "u1", "expect": 422, "compare": "full"},
    {"n": 10, "d": "wallet_code=WALLET_FUEL exact filter",
     "url": "/api/wallet-recharges?wallet_code=WALLET_FUEL", "hk": "u1", "expect": 200, "compare": "full"},
    {"n": 11, "d": "wallet_code='' blank → omitted",
     "url": "/api/wallet-recharges?wallet_code=", "hk": "u1", "expect": 200, "compare": "full"},
    {"n": 12, "d": "wallet_code=NON_EXISTENT → 200 []",
     "url": "/api/wallet-recharges?wallet_code=NON_EXISTENT", "hk": "u1", "expect": 200, "compare": "full"},
    {"n": 13, "d": "date_from only · rows >= 2026-05-03",
     "url": "/api/wallet-recharges?date_from=2026-05-03", "hk": "u1", "expect": 200, "compare": "full"},
    {"n": 14, "d": "date_to only · rows <= 2026-05-02",
     "url": "/api/wallet-recharges?date_to=2026-05-02", "hk": "u1", "expect": 200, "compare": "full"},
    {"n": 15, "d": "both bounds · inclusive range",
     "url": "/api/wallet-recharges?date_from=2026-05-02&date_to=2026-05-03", "hk": "u1", "expect": 200, "compare": "full"},
    {"n": 16, "d": "blank date bounds omitted",
     "url": "/api/wallet-recharges?date_from=&date_to=", "hk": "u1", "expect": 200, "compare": "full"},
    {"n": 17, "d": "no auth (precedes 422)",
     "url": "/api/wallet-recharges?include_deleted=FOO", "hk": "none", "expect": 401, "compare": "full"},
    {"n": 18, "d": "invalid bearer → 401 Invalid session",
     "url": "/api/wallet-recharges", "hk": "bad", "expect": 401, "compare": "full"},
    {"n": 19, "d": "expired bearer → 401 Session expired",
     "url": "/api/wallet-recharges", "hk": "expired", "expect": 401, "compare": "full"},
    {"n": 20, "d": "owned X-Company-Id · co-a-alt → [r-alt]",
     "url": "/api/wallet-recharges", "hk": "u1",
     "extra_hdr": {"X-Company-Id": "co-a-alt"}, "expect": 200, "compare": "full"},
    {"n": 21, "d": "unowned X-Company-Id · fallback co-a",
     "url": "/api/wallet-recharges", "hk": "u1",
     "extra_hdr": {"X-Company-Id": "co-b"}, "expect": 200, "compare": "full"},
    {"n": 22, "d": "cross-user · u2 · own rows only",
     "url": "/api/wallet-recharges", "hk": "u2", "expect": 200, "compare": "full"},
]
...
async def run() -> int:
    print(f"[gate7c] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate7c] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate7c_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate7c_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 7c · LIVE PARITY MATRIX (WalletRecharges list)")
        print("=" * 72)
        for r in results:
