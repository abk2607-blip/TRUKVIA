"""Reproject mechanic_payment documents through the REAL Python implementation.

Used by scripts/fin-mechanic-parity.ts as the reference side.

    python fin_mechanic_driver.py <db_name> <spec_json_path>
...
async def main():
    ...
    for _ in range(2):  # twice: reprojection must be idempotent
        for source_id, uid, cid in spec:
            reports.append(
                await hook_after_source_write(uid, cid, "mechanic_payment", source_id)
            )
