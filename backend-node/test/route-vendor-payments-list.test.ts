/**
 * TRUKVIA · Phase-3 · Gate-6z · VendorPayment list read-only shadow — Vitest.
 *
 * Covers:
 *   GET /api/vendors/{vid}/payments
 *
 * Fake-DB write-forbid pattern inlined (mirrors Gate 6t/6u/6v/6w/6x). `$ne`
 * matcher inlined with Mongo semantics (missing key returns TRUE for
 * `$ne:true`).
 * Gate 6z exercises:
 *   * SINGLE soft-delete predicate `is_deleted: {$ne: true}`.
 *   * Projection strips BOTH `_id` and `user_id`.
 *   * `activeCompanyId()` consumption (X-Company-Id changes rowset).
 *   * 5000-row cap.
 *   * date DESC ordering.
 *   * NO 404 branch: unknown vid / wrong-company vid → 200 [].
 */
...
interface State {
  ...
  vendor_payments: Record<string, unknown>[];
  writes: string[];
}
...
function fakeDb(state: State): Db {
  const rowsOf = (name: string): Record<string, unknown>[] =>
    ...
    : name === 'vendor_payments' ? state.vendor_payments
    : [];
...
function vp(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    id: 'vp-x', user_id: 'u1', company_id: 'co-a',
    vendor_id: 'ven-1',
    date: D1, amount: 2500, type: 'payment_out',
    mode: 'Bank', ref_no: 'RN-1',
    remarks: 'clean pay', is_deleted: false,
    ...overrides,
  };
}
...
function makeState(): State {
  return {
    ...
    vendor_payments: [
      // Active co-a / ven-1 payments ($ne:true predicate cleared).
      vp({ id: 'p1', date: D1, is_deleted: false }),
      vp({ id: 'p2', date: D2, is_deleted: false }),
      vp({ id: 'p3', date: D3 }), // is_deleted MISSING → included
      vp({ id: 'p4', date: D4, is_deleted: null }), // null → included
      // Excluded by $ne:true predicate.
      vp({ id: 'p-del', date: D1, is_deleted: true }),
      // Different vendor same company — must NOT appear.
      vp({ id: 'p-ven2', vendor_id: 'ven-2', date: D1 }),
      // Same vendor different company (owned & unowned).
      vp({ id: 'p-alt',  company_id: 'co-a-alt', date: D1 }),
      vp({ id: 'p-u2',   user_id: 'u2', company_id: 'co-b', date: D1 }),
    ],
    writes: [],
  };
}
...
const VEN1_ACTIVE_IDS = ['p1', 'p2', 'p3', 'p4']; // DESC by date

describe('Gate-6z · VendorPayment list read-only shadow', () => {
  let state: State;
  ...
  // ── HAPPY / EMPTY ─────────────────────────────────────────────────
  it('1 successful list · ven-1 · co-a · four active rows DESC by date', async () => {
    const r = await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(VEN1_ACTIVE_IDS);
  });

  it('2 empty list · different vendor with no payments → 200 []', async () => {
    const r = await get('/api/vendors/ven-empty/payments', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
    expect(r.json()).toEqual([]);
  });

  it('3 unknown vendor → 200 [] (no 404 vendor-existence lookup)', async () => {
    const r = await get('/api/vendors/does-not-exist/payments', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
    expect(r.json()).toEqual([]);
  });

  it('4 wrong-company vendor (u1 asks for u2/co-b vendor) → 200 [] no leakage', async () => {
    const r = await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).not.toContain('p-u2');
  });

  it('5 same vendor across companies · u2 sees only own co-b row', async () => {
    const r = await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u2' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
  ...
  it('6 owned X-Company-Id override · co-a-alt → p-alt only', async () => {
    const r = await get('/api/vendors/ven-1/payments', {
  ...
  it('13 projection strips BOTH _id and user_id', async () => {
    const r = await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u1' });
  ...
  it('14 date DESC ordering · p1 > p2 > p3 > p4', async () => {
    const r = await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u1' });
  ...
  it('15 5000 cap · exactly 5000 returned when >5000 rows exist', async () => {
    state.vendor_payments.length = 0;
    for (let i = 0; i < 5001; i++) {
      state.vendor_payments.push(vp({
  ...
  it('17 invalid bearer → 401 Invalid session', async () => {
    const r = await get('/api/vendors/ven-1/payments', { authorization: 'Bearer nope' });
    expect(r.statusCode).toBe(401);
    expect(r.json()).toEqual({ detail: 'Invalid session' });
  });

  it('18 expired session → 401 Session expired', async () => {
    const r = await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-expired' });
    expect(r.statusCode).toBe(401);
    expect(r.json()).toEqual({ detail: 'Session expired' });
  });

  // ── ZERO-WRITE AGGREGATE ───────────────────────────────────────────
  it('19 zero Node writes across every exercised branch', async () => {
    await get('/api/vendors/ven-1/payments');
    await get('/api/vendors/ven-1/payments', { authorization: 'Bearer nope' });
    await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-expired' });
    await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u1' });
    await get('/api/vendors/does-not-exist/payments', { authorization: 'Bearer tok-u1' });
    await get('/api/vendors/ven-empty/payments', { authorization: 'Bearer tok-u1' });
    await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u2' });
    await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u1', 'x-company-id': 'co-a-alt' });
    await get('/api/vendors/ven-1/payments', { authorization: 'Bearer tok-u1', 'x-company-id': 'co-b' });
