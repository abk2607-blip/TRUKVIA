/**
 * TRUKVIA · Phase-3 · Gate-7b · WalletTransfers list read-only shadow — Vitest.
 *
 * Covers:
 *   GET /api/wallet-transfers
 *
 * STRICT SUBSET of Gate 7a. No `wallet_code` query param — client-supplied
 * `?wallet_code=...` MUST be silently ignored. All other query axes
 * (date_from / date_to / include_deleted) reuse the Gate 7a semantics.
 * Fake-DB write-forbid pattern + $ne/$gte/$lte matcher inlined route-locally.
 */
...
interface State {
  ...
  wallet_transfers: Record<string, unknown>[];
  writes: string[];
}
...
function fakeDb(state: State): Db {
  const rowsOf = (name: string): Record<string, unknown>[] =>
    ...
    : name === 'wallet_transfers' ? state.wallet_transfers
    : [];
...
function wt(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
...
function makeState(): State {
  return {
    ...
    wallet_transfers: [
      wt({ id: 't1', date: D1, is_deleted: false }),
      wt({ id: 't2', date: D2, is_deleted: false }),
      wt({ id: 't3', date: D3 }),                       // is_deleted MISSING → included
      wt({ id: 't4', date: D4, is_deleted: null }),     // null → included
      wt({ id: 't-del', date: D1, is_deleted: true }),  // excluded unless include_deleted=true
      wt({ id: 't-alt', company_id: 'co-a-alt', date: D1 }),
      wt({ id: 't-u2',  user_id: 'u2', company_id: 'co-b', date: D1 }),
    ],
    writes: [],
  };
}
...
const CO_A_ACTIVE_IDS = ['t1', 't2', 't3', 't4'];

describe('Gate-7b · WalletTransfers list read-only shadow', () => {
  let state: State;
  ...
  it('1 default include_deleted=false · four active rows DESC', async () => {
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer tok-u1' });
  ...
  it('2 omitted include_deleted === false · deleted excluded', async () => {
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).not.toContain('t-del');
  });

  // ── include_deleted ────────────────────────────────────────────────
  it('3 include_deleted=true · deleted included', async () => {
    const r = await get('/api/wallet-transfers?include_deleted=true', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toContain('t-del');
  });

  it('4 include_deleted=false · deleted excluded', async () => {
    const r = await get('/api/wallet-transfers?include_deleted=false', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(CO_A_ACTIVE_IDS);
  });
  ...
  it('5 True token variants · true/True/TRUE/1/yes/Yes/YES/on/On/ON', async () => {
    for (const t of ['true', 'True', 'TRUE', '1', 'yes', 'Yes', 'YES', 'on', 'On', 'ON']) {
      const r = await get(`/api/wallet-transfers?include_deleted=${t}`, { authorization: 'Bearer tok-u1' });
      expect(r.statusCode).toBe(200);
      const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
      expect(ids).toContain('t-del');
    }
  ...
  // ── invalid bool → 422 (byte-exact envelope) ───────────────────────
  it('7 invalid include_deleted="" → 422 Pydantic bool_parsing envelope', async () => {
    const r = await get('/api/wallet-transfers?include_deleted=', { authorization: 'Bearer tok-u1' });
  ...
  it('8 invalid include_deleted=FOO → 422 Pydantic envelope', async () => {
    const r = await get('/api/wallet-transfers?include_deleted=FOO', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(422);
    expect(r.json()).toEqual({
      detail: [{
        type: 'bool_parsing',
  ...
  it('9a no auth → 401 Not authenticated', async () => {
    const r = await get('/api/wallet-transfers');
    expect(r.statusCode).toBe(401);
    expect(r.json()).toEqual({ detail: 'Not authenticated' });
  });

  it('9b invalid bearer → 401 Invalid session', async () => {
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer nope' });
    expect(r.statusCode).toBe(401);
    expect(r.json()).toEqual({ detail: 'Invalid session' });
  });

  it('9c expired session → 401 Session expired', async () => {
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer tok-expired' });
    expect(r.statusCode).toBe(401);
    expect(r.json()).toEqual({ detail: 'Session expired' });
  });

  it('10 auth precedence · no bearer + invalid include_deleted → 401 (not 422)', async () => {
    const r = await get('/api/wallet-transfers?include_deleted=FOO');
    expect(r.statusCode).toBe(401);
    expect(r.json()).toEqual({ detail: 'Not authenticated' });
  });

  // ── date range ─────────────────────────────────────────────────────
  it('11 date_from only · rows >= 2026-05-03', async () => {
    const r = await get('/api/wallet-transfers?date_from=2026-05-03', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['t1', 't2']);
  });

  it('12 date_to only · rows <= 2026-05-02', async () => {
    const r = await get('/api/wallet-transfers?date_to=2026-05-02', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['t3', 't4']);
  });

  it('13 both date bounds · inclusive lexicographic range', async () => {
    const r = await get('/api/wallet-transfers?date_from=2026-05-02&date_to=2026-05-03', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['t2', 't3']);
  });

  it('14 blank date bounds → both omitted', async () => {
    const r = await get('/api/wallet-transfers?date_from=&date_to=', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(CO_A_ACTIVE_IDS);
  });

  it('15 inclusive both bounds · all 4 active rows', async () => {
    const r = await get('/api/wallet-transfers?date_from=2026-05-01&date_to=2026-05-04', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(CO_A_ACTIVE_IDS);
  });

  // ── projection / sort / cap / empty ────────────────────────────────
  it('16 projection strips _id and user_id', async () => {
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer tok-u1' });
  ...
  it('17 date DESC · t1 > t2 > t3 > t4', async () => {
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer tok-u1' });
    const dates = (r.json() as Record<string, unknown>[]).map((x) => x['date']);
    expect(dates).toEqual([D1, D2, D3, D4]);
  });

  it('18 5000 cap · exactly 5000 returned when >5000 rows exist', async () => {
    state.wallet_transfers.length = 0;
    for (let i = 0; i < 5001; i++) {
      state.wallet_transfers.push(wt({
  ...
  it('19 empty result · date_from far future → 200 []', async () => {
    const r = await get('/api/wallet-transfers?date_from=2099-01-01', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
    expect(r.json()).toEqual([]);
  });

  // ── isolation & X-Company-Id ───────────────────────────────────────
  it('20 cross-user/company isolation · u2 sees only own rows', async () => {
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer tok-u2' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['t-u2']);
  });

  it('21 owned X-Company-Id override · co-a-alt → t-alt only', async () => {
    const r = await get('/api/wallet-transfers', {
      authorization: 'Bearer tok-u1', 'x-company-id': 'co-a-alt',
    });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['t-alt']);
  });

  it('22 unowned X-Company-Id (co-b for u1) → fallback to default co-a', async () => {
    const r = await get('/api/wallet-transfers', {
  ...
  it('23 no X-Company-Id header · default company (co-a)', async () => {
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(CO_A_ACTIVE_IDS);
  });

  // ── ignored extra query params ─────────────────────────────────────
  it('24 unknown extra query parameter silently ignored', async () => {
    const r = await get('/api/wallet-transfers?foo=bar&limit=10', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(CO_A_ACTIVE_IDS);
  });

  it('25 wallet_code ignored · Python route does not define it', async () => {
    // Even a value that WOULD have filtered in Gate 7a must have zero
    // effect here — the Node target must not accept wallet_code for
    // wallet-transfers.
    const r = await get('/api/wallet-transfers?wallet_code=WALLET_FUEL', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(CO_A_ACTIVE_IDS); // all 4 active rows, not filtered
  });

  it('26 no 404 branch · empty state → 200 []', async () => {
    state.wallet_transfers.length = 0;
    const r = await get('/api/wallet-transfers', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
    expect(r.json()).toEqual([]);
  ...
  it('27 zero Node writes across every exercised branch', async () => {
    ...
    await get('/api/wallet-transfers', { authorization: 'Bearer nope' });
    await get('/api/wallet-transfers', { authorization: 'Bearer tok-expired' });
    await get('/api/wallet-transfers?include_deleted=FOO', { authorization: 'Bearer tok-u1' });
    await get('/api/wallet-transfers?include_deleted=', { authorization: 'Bearer tok-u1' });
    await get('/api/wallet-transfers?include_deleted=true', { authorization: 'Bearer tok-u1' });
    await get('/api/wallet-transfers?include_deleted=false', { authorization: 'Bearer tok-u1' });
    await get('/api/wallet-transfers?date_from=2026-05-02&date_to=2026-05-03', { authorization: 'Bearer tok-u1' });
    await get('/api/wallet-transfers?wallet_code=WALLET_FUEL&foo=bar', { authorization: 'Bearer tok-u1' });
    await get('/api/wallet-transfers', { authorization: 'Bearer tok-u2' });
    await get('/api/wallet-transfers', { authorization: 'Bearer tok-u1', 'x-company-id': 'co-a-alt' });
