/**
 * TRUKVIA · Phase-3 · Gate-6o · Vendor payment corrections read-only shadow — Vitest.
 *
 * Covers:
 *   GET /api/vendor-payments/{pid}/corrections
 *
 * Fake-DB write-forbid pattern reused inline (no shared/locked test
 * infrastructure touched). Gate-6o-only fixture surface adds a
 * `payment_corrections` rowset spanning vendor / supplier /
 * mechanic / driver payment types to prove cross-type isolation.
 */
...
function corr(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    payment_type: 'vendor', payment_id: 'pay-1',
...
function makeState(): State {
  return {
    ...
    payment_corrections: [
      // pay-1 vendor corrections under u1/co-a (three, out-of-order created).
      corr({ id: 'c-v3', payment_id: 'pay-1', correction_index: 3, correction_reason: 'third' }),
      corr({ id: 'c-v1', payment_id: 'pay-1', correction_index: 1, correction_reason: 'first', extra_field: 'preserved' }),
      corr({ id: 'c-v2', payment_id: 'pay-1', correction_index: 2, correction_reason: 'second' }),
      ...
      corr({ id: 'c-mech',  payment_type: 'mechanic', payment_id: 'pay-1', correction_index: 1, correction_reason: 'mechanic-crumb' }),
      corr({ id: 'c-driv',  payment_type: 'driver',   payment_id: 'pay-1', correction_index: 1, correction_reason: 'driver-crumb'   }),
      // pay-2 vendor corrections — different pid.
      corr({ id: 'c-p2-v1', payment_id: 'pay-2', correction_index: 1, correction_reason: 'p2-first' }),
...
describe('Gate-6o · Vendor payment corrections read-only shadow', () => {
  let state: State;
  ...
  it('1 happy list · returns only vendor corrections for pid under u1/co-a', async () => {
    const r = await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-owner' });
    expect(r.statusCode).toBe(200);
    const rows = r.json() as Record<string, unknown>[];
    expect(rows.map((x) => x['id'])).toEqual(['c-v1', 'c-v2', 'c-v3']);
  });

  it('2 unknown pid → 200 []', async () => {
    const r = await get('/api/vendor-payments/no-such-pid/corrections', { authorization: 'Bearer tok-owner' });
    expect(r.statusCode).toBe(200);
    expect(r.json()).toEqual([]);
  });

  it('3 cross-payment-type isolation · supplier/mechanic/driver rows excluded', async () => {
    const r = await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-owner' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).not.toContain('c-supp');
    expect(ids).not.toContain('c-mech');
    expect(ids).not.toContain('c-driv');
    for (const row of r.json() as Record<string, unknown>[]) {
      expect(row['payment_type']).toBe('vendor');
    }
  });
  ...
  it('5 cross-company isolation · co-a-alt row not returned under default co-a', async () => {
    ...
    expect(ids).toEqual(['c-v1', 'c-v2', 'c-v3']);
  });

  it('6 owned X-Company-Id override → alt-company row', async () => {
    const r = await get('/api/vendor-payments/pay-1/corrections', {
  ...
  it('12 pay-2 returns only its own vendor corrections', async () => {
    const r = await get('/api/vendor-payments/pay-2/corrections', { authorization: 'Bearer tok-owner' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['c-p2-v1']);
  });

  it('13 extra fields preserved verbatim (no re-shaping)', async () => {
    const r = await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-owner' });
    const rows = r.json() as Record<string, unknown>[];
    const c1 = rows.find((x) => x['id'] === 'c-v1')!;
  ...
  it('15 invalid bearer → 401 Invalid session', async () => {
    const r = await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer nope' });
    expect(r.statusCode).toBe(401);
    expect(r.json()).toEqual({ detail: 'Invalid session' });
  });

  it('16 expired session → 401 Session expired', async () => {
    const r = await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-expired' });
    expect(r.statusCode).toBe(401);
    expect(r.json()).toEqual({ detail: 'Session expired' });
  });

  it('17 zero Node business writes across every branch', async () => {
    await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-owner' });
    await get('/api/vendor-payments/pay-2/corrections', { authorization: 'Bearer tok-owner' });
    await get('/api/vendor-payments/no-such-pid/corrections', { authorization: 'Bearer tok-owner' });
    await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-owner', 'x-company-id': 'co-a-alt' });
    await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-owner', 'x-company-id': 'co-b' });
    await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-u2' });
    await get('/api/vendor-payments/pay-1/corrections');
    await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer nope' });
    await get('/api/vendor-payments/pay-1/corrections', { authorization: 'Bearer tok-expired' });
