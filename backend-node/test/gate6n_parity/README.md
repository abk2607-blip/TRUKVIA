# Phase 3 · Gate 6n · Live Python↔Node parity harness
...
* Isolated timestamped disposable DB: `trukvia_gate6n_parity_<timestamp>`.
...
db.payment_corrections.find(
    {"user_id": uid, "company_id": cid,
     "payment_type": "mechanic", "payment_id": pid},
    {"_id": 0, "user_id": 0},
...
* Filter EXACTLY `{ user_id, company_id, payment_type: "mechanic", payment_id }`.
...
* Cross-payment-type isolation intrinsic via `payment_type: "mechanic"`.

## Divergence from locked Gate-2

The locked `backend-node/src/routes/supplier-payments-corrections.ts`
uses an unbounded `.toArray()`. Gate-6n MUST use `.limit(1000)` to
...
1. happy list (mechanic `pay-1` — three rows returned ASC by `correction_index`)
2. unknown pid → `200 []`
3. second pid (`pay-2`) returns its own mechanic correction only
...
Python: `8181`  ·  Node: `8182`  (non-overlapping with prior gates).

## Run

```
cd /app/backend-node && npx tsc && cd /app
python3 backend-node/test/gate6n_parity/harness.py
```

Result JSON is written to `/tmp/gate6n_parity_results.json`.

## Writer boundary (Gate-7)

`POST /api/mechanic-payments/{pid}/correct-amount` (Python
`apply_amount_reversal_new` → `hook_after_source_write` + ledger
reversal + audit) is OUT OF SCOPE for Gate 6n and remains
