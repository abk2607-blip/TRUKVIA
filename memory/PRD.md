...
- [x] **Iter105 · Customer Policy Change Workflow — Phase A (Preview + Apply)** (Feb 2026 — awaiting user UAT before Revert phase)
  - **Backend router** `/app/backend/routers/policy_changes.py` — three endpoints under `/api/policy-changes`:
    - `POST /preview` — READ-ONLY dry-run. Returns eligible pending trips (uninvoiced + non-historical + `date >= effective_from`) with old vs new snapshot + old vs new engine-recomputed financials + Δ freight/Δ shortage/Δ net-settlement per trip. Both sides run through `services._compute_trip` on the same code path so deltas are apples-to-apples. Skipped counts (invoiced, historical) surfaced in the response for transparency.
    - `POST /apply` — Persists the customer master update, then for each explicitly listed trip_id in the request: re-snapshots `applied_*` from CURRENT masters (Customer + Product + Supplier), recomputes via `services._compute_trip`, and writes ONLY the applied_* + recomputed financial fields. Non-selected trips are guaranteed untouched. Invoiced / historical / out-of-range trips are hard-skipped and reported in `skipped`. Creates a `policy_change_events` row with old_policy, new_policy, per_trip_deltas, actor, status=`applied`. Also drops an `audit_logs` crumb (`action=policy_change_apply`).
    - `GET /policy-changes?customer_id=...` — Returns the customer's policy-change history (Phase B Revert will read from here).
  - **Contract** — Effective Date is MANDATORY (400 without it on both preview + apply). `apply_to_previous=False` never touches trips. Company + user_id isolation enforced on every query. Supplier `applied_supplier_shortage_limit_kg` is preserved verbatim for non-supplier trips so supplier-side settlement stays independent.
  - **Frontend** — New `/app/frontend/src/components/PolicyChangeDialog.jsx`. Wired into the Customers.jsx edit-form submit path AND an explicit `Review policy · pending trips…` button in the "Customer Shortage Rule" section. The dialog opens whenever an existing customer with a `shortage_config.effective_from` set is saved OR the explicit button is clicked — so a user can re-apply the CURRENT policy to old trips whose snapshot is empty/stale even when the form fields match the DB exactly. Two-stage UX — Stage 1 shows Current vs New policy summary + Effective Date + "Apply to Previous Eligible Trips" checkbox (OFF by default). Stage 2 (only when the checkbox flips ON) fetches `/preview` and renders the eligible-trips table with per-row selection, per-row old vs new Freight/Shortage and Δ Net Settlement, and an optional Reason field. `data-testid`s: `policy-change-dialog`, `apply-previous-toggle`, `policy-change-preview-table`, `select-trip-{id}`, `policy-change-save`, `review-policy-pending-trips-btn`.
  - **Tests** — `test_iter105_policy_change_workflow.py` (8/8 pass): effective-from mandatory, preview lists only eligible + skips invoiced/out-of-range, preview is read-only, apply recomputes via engine to the expected engineered math (33.2 × 0.5% = 0.166 MT allowed → full 0.4 MT × ₹62500 = ₹25 000 with `full_after_limit`), OFF path never mutates trips, invoiced/historical/out-of-range trips skipped even when listed, non-selected eligible trips guaranteed untouched, policy_change_events row captured with per-trip deltas and returned by the history endpoint.
  - **Status** — Phase A DONE. Phase B (Revert with mandatory reason + protected UX) is intentionally NOT built yet — waiting for the user's live UAT sign-off on Preview + Apply before continuing.
