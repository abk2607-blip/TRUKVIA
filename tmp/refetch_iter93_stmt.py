...
H = {"Authorization": "Bearer test_session_bitumen_2026"}
requests.post(f"{API}/api/auth/demo-login", timeout=30)
cid = requests.get(f"{API}/api/companies", headers=H, timeout=30).json()[0]["id"]
H["X-Company-Id"] = cid
trips = requests.get(f"{API}/api/trips?limit=500&vehicle_type=supplier", headers=H, timeout=45).json()
if isinstance(trips, dict): trips = trips.get("items") or trips.get("trips") or []
sn = ""
for t in trips:
    if t.get("supplier_name") and t.get("supplier_name","").startswith("PEDDINTI"):
