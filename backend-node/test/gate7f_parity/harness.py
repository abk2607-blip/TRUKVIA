"""Phase 3 · Gate 7f · live Python↔Node parity harness for
Approvals pending-count read.

Covers:
  * GET /api/approvals/summary/pending

STRICT UAT-DATA PRESERVATION:
  Isolated timestamped DB `trukvia_gate7f_parity_<ts>`, dropped in
...
NEW parity axes for Gate 7f:
  ...
  * First use of `count_documents` operator in the Node shadow.
  * Fixed-shape response {"count": <int>}.
...
DB = f"trukvia_gate7f_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8218, 8219
...
def _apr(**kw: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
        ...
        "entity_kind": "trip", "created_at": now_iso(),
    }
    base.update(kw)
    return base
...
def fixtures() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        "approvals": [
            ...
            _apr(id="a-approved",  status="APPROVED"),
            _apr(id="a-rejected",  status="REJECTED"),
            _apr(id="a-withdrawn", status="WITHDRAWN"),
            _apr(id="a-posted",    status="POSTED"),
            # u1 / co-a-alt — 1 PENDING (company isolation)
            _apr(id="a-alt", company_id="co-a-alt"),
            # u2 / co-b — 2 PENDING (cross-user isolation)
            _apr(id="a-u2-1", user_id="u2", company_id="co-b"),
            _apr(id="a-u2-2", user_id="u2", company_id="co-b"),
        ],
    }
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate7f_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate7f_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    {"n":  1, "d": "no auth → 401 Not authenticated",
     "url": "/api/approvals/summary/pending", "hk": "none", "expect": 401, "compare": "full"},
    {"n":  2, "d": "invalid bearer → 401 Invalid session",
     "url": "/api/approvals/summary/pending", "hk": "bad", "expect": 401, "compare": "full"},
    {"n":  3, "d": "expired bearer → 401 Session expired",
     "url": "/api/approvals/summary/pending", "hk": "expired", "expect": 401, "compare": "full"},
    {"n":  4, "d": "u1 · default co-a → count 3 PENDING",
     "url": "/api/approvals/summary/pending", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  5, "d": "owned X-Company-Id co-a-alt → count 1 (alt-company row)",
     "url": "/api/approvals/summary/pending", "hk": "u1",
     "extra_hdr": {"X-Company-Id": "co-a-alt"}, "expect": 200, "compare": "full"},
    {"n":  6, "d": "unowned X-Company-Id co-b → fallback co-a → count 3",
     "url": "/api/approvals/summary/pending", "hk": "u1",
     "extra_hdr": {"X-Company-Id": "co-b"}, "expect": 200, "compare": "full"},
    {"n":  7, "d": "cross-user u2 · co-b → count 2 PENDING (isolation)",
     "url": "/api/approvals/summary/pending", "hk": "u2", "expect": 200, "compare": "full"},
]
...
async def run() -> int:
    print(f"[gate7f] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate7f] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate7f_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate7f_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 7f · LIVE PARITY MATRIX (Approvals pending-count)")
        print("=" * 72)
        for r in results:
