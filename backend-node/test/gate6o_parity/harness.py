"""Phase 3 · Gate 6o · live Python↔Node parity harness for Vendor
...
STRICT UAT-DATA PRESERVATION:
  Uses an isolated timestamped DB (`trukvia_gate6o_parity_<ts>`) that
...
DB = f"trukvia_gate6o_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8183, 8184
...
def fixtures() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        "payment_corrections": [
            # pay-1 vendor corrections under u1/co-a — deliberately out-of-order.
            _corr(id="c-v3", payment_id="pay-1", correction_index=3, correction_reason="third"),
            _corr(id="c-v1", payment_id="pay-1", correction_index=1, correction_reason="first", extra_field="preserved"),
            _corr(id="c-v2", payment_id="pay-1", correction_index=2, correction_reason="second"),
            ...
            _corr(id="c-mech", payment_type="mechanic", payment_id="pay-1", correction_index=1, correction_reason="mechanic-crumb"),
            ...
            _corr(id="c-p2-v1", payment_id="pay-2", correction_index=1, correction_reason="p2-first"),
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6o_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6o_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    {"n":  1, "d": "happy list · vendor pay-1 · ASC by correction_index",
     "url": "/api/vendor-payments/pay-1/corrections", "hk": "owner", "expect": 200, "compare": "full"},
    {"n":  2, "d": "unknown pid → 200 []",
     "url": "/api/vendor-payments/no-such-pid/corrections", "hk": "owner", "expect": 200, "compare": "full"},
    {"n":  3, "d": "second pid (pay-2) returns its own vendor corrections",
     "url": "/api/vendor-payments/pay-2/corrections", "hk": "owner", "expect": 200, "compare": "full"},
    {"n":  4, "d": "cross-user isolation (u2 view — u2's own row)",
     "url": "/api/vendor-payments/pay-1/corrections", "hk": "u2", "expect": 200, "compare": "full"},
    {"n":  5, "d": "owned X-Company-Id override → alt-company row",
     "url": "/api/vendor-payments/pay-1/corrections", "hk": "owner",
    ...
    {"n":  7, "d": "no auth → 401 Not authenticated",
     "url": "/api/vendor-payments/pay-1/corrections", "hk": "none", "expect": 401, "compare": "full"},
    {"n":  8, "d": "invalid bearer → 401 Invalid session",
     "url": "/api/vendor-payments/pay-1/corrections", "hk": "bad", "expect": 401, "compare": "full"},
    {"n":  9, "d": "expired session → 401 Session expired",
     "url": "/api/vendor-payments/pay-1/corrections", "hk": "expired", "expect": 401, "compare": "full"},
]
...
async def run() -> int:
    print(f"[gate6o] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate6o] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate6o_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate6o_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 6o · LIVE PARITY MATRIX (Vendor payment corrections)")
        print("=" * 72)
        for r in results:
