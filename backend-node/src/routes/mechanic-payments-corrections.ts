...
/**
 * TRUKVIA · Phase-3 · Gate-6n · Mechanic payment corrections read-only shadow.
 *
 *   GET /api/mechanic-payments/{pid}/corrections
 *
 * Faithful shadow of:
 *   backend/routers/mechanic_ledger.py::get_mechanic_payment_corrections
 *   (lines 117–122)
 ...
 *
 * Cross-payment-type isolation is intrinsic: supplier / vendor /
 * driver correction rows sharing the same `payment_id` are excluded
 * because `payment_type` is fixed to the string `"mechanic"` at the
 * filter level.
 *
 ...
 *   `list_corrections` caps at 1000 via `.to_list(1000)`; this Gate-6n
 ...
 *   ledger reversal + audit) is out of scope for Gate 6n and remains
...
// ── Handler registrar ─────────────────────────────────────────────────
export async function registerMechanicPaymentsCorrectionsReadRoutes(
  app: FastifyInstance,
  deps: { db: Db },
): Promise<void> {
  const { db } = deps;

  // migration-allowlisted: phase-3-gate-6n (read-only)
