"""Phase 3 · Gate 6y · live Python↔Node parity harness for
DriverPayment list reads.

Covers:
  * GET /api/drivers/{did}/payments

STRICT UAT-DATA PRESERVATION:
  Uses an isolated timestamped DB (`trukvia_gate6y_parity_<ts>`) that
  is dropped in `finally`. Never touches `test_database` nor any
  existing TRUKVIA UAT tenant / login.

Class-C stance:
  Pure-read in Python. Handler performs exactly two reads:
...
Gate-6y NEW dimensions:
  * Cross-collection `drivers.find_one` pre-check → 404 literal
    `"Driver not found"` on miss.
  * Projection strips ONLY `_id`; `user_id` REMAINS in the response.
  * NO `.to_list(5000)` cap — unbounded async cursor.
"""
...
DB = f"trukvia_gate6y_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8204, 8205
...
def _dp(**kw: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
        "id": "dp-x", "user_id": "u1", "company_id": "co-a",
        "driver_id": "drv-1",
        "date": D1, "amount": 1000.0, "type": "payment_out",
...
def fixtures() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        "driver_payments": [
            _dp(id="x1", date=D1, ref_no="DR-1"),
            _dp(id="x2", date=D2, ref_no="DR-2"),
            _dp(id="x-del", date=D1, ref_no="DR-DEL", is_deleted=True),
            _dp(id="x-alt", company_id="co-a-alt", date=D1, ref_no="DR-ALT"),
            _dp(id="x-u2", user_id="u2", company_id="co-b", driver_id="drv-u2", date=D1, ref_no="DR-U2"),
        ],
    }
...
TRACKED = ("users", "companies", "customers", "trips",
           ...
           "fin_hook_failures", "vendors", "mechanics", "drivers",
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6y_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6y_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    {"n":  1, "d": "valid driver with payments · drv-1 · [x1,x2] DESC (user_id present, deleted excluded)",
     "url": "/api/drivers/drv-1/payments", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  2, "d": "valid driver with no payments · drv-empty → 200 []",
     "url": "/api/drivers/drv-empty/payments", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  3, "d": "unknown driver → 404 exact literal",
     "url": "/api/drivers/does-not-exist/payments", "hk": "u1", "expect": 404, "compare": "full"},
    {"n":  4, "d": "wrong-company driver · drv-u2 belongs to co-b → same 404",
     "url": "/api/drivers/drv-u2/payments", "hk": "u1", "expect": 404, "compare": "full"},
    {"n":  5, "d": "cross-user · u2 · drv-u2 → [x-u2]",
     "url": "/api/drivers/drv-u2/payments", "hk": "u2", "expect": 200, "compare": "full"},
    {"n":  6, "d": "owned X-Company-Id override · co-a-alt · drv-1 → [x-alt]",
     "url": "/api/drivers/drv-1/payments", "hk": "u1",
    ...
    {"n":  8, "d": "no auth → 401 Not authenticated",
     "url": "/api/drivers/drv-1/payments", "hk": "none", "expect": 401, "compare": "full"},
    {"n":  9, "d": "invalid bearer → 401 Invalid session",
     "url": "/api/drivers/drv-1/payments", "hk": "bad", "expect": 401, "compare": "full"},
    {"n": 10, "d": "expired bearer → 401 Session expired",
     "url": "/api/drivers/drv-1/payments", "hk": "expired", "expect": 401, "compare": "full"},
]
...
async def run() -> int:
    print(f"[gate6y] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate6y] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate6y_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate6y_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 6y · LIVE PARITY MATRIX (DriverPayments list)")
        print("=" * 72)
        for r in results:
