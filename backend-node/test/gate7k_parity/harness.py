"""Phase 3 · Gate 7k · live Python↔Node parity harness for Toll-import lookup.

Covers:
  * GET /api/toll-import/lookup

Class-C stance:
  Pure-read in Python. Handler executes at most two
    db.expenses.find_one(q, {_id:0, user_id:0})
  calls. Zero writes / audits / backfill / recompute / hooks.

CRITICAL parity axis:
  Python `f"...source_txn_ref={txn_ref!r}..."` uses `repr()` — single-
  quoted by default, escapes `\\`, `'`, control chars via `\\n` / `\\r`
  / `\\t` / `\\xNN`, switches to double-quote wrap when the string
...
DB = f"trukvia_gate7k_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8228, 8229
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate7k_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate7k_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    # Auth
    {"n":  1, "d": "no bearer + valid query → 401 Not authenticated",
     "url": "/api/toll-import/lookup?txn_ref=TXN-IDFC-100", "hk": "none", "expect": 401},
    {"n":  2, "d": "invalid bearer → 401 Invalid session",
     "url": "/api/toll-import/lookup?txn_ref=X", "hk": "bad", "expect": 401},
    {"n":  3, "d": "expired bearer → 401 Session expired",
     "url": "/api/toll-import/lookup?txn_ref=X", "hk": "expired", "expect": 401},

    # 400 required-param (auth-then-400 ordering)
    {"n":  4, "d": "missing txn_ref → 400 required message",
     "url": "/api/toll-import/lookup", "hk": "u1", "expect": 400},
    {"n":  5, "d": "blank txn_ref= → 400 required message",
     "url": "/api/toll-import/lookup?txn_ref=", "hk": "u1", "expect": 400},
    {"n":  6, "d": "no bearer + missing txn_ref → 401 (auth wins over 400)",
     "url": "/api/toll-import/lookup", "hk": "none", "expect": 401},

    # Primary hit
    {"n":  7, "d": "primary hit by source_txn_ref (no vendor)",
     "url": "/api/toll-import/lookup?txn_ref=TXN-IDFC-100", "hk": "u1", "expect": 200},
    {"n":  8, "d": "primary hit with vendor=idfc (matches source)",
     "url": "/api/toll-import/lookup?txn_ref=TXN-IDFC-100&vendor=idfc", "hk": "u1", "expect": 200},
    {"n":  9, "d": "vendor lowercased: vendor=IDFC → source=idfc match",
     "url": "/api/toll-import/lookup?txn_ref=TXN-IDFC-100&vendor=IDFC", "hk": "u1", "expect": 200},
    {"n": 10, "d": "vendor=livq wrong-vendor → 404 (repr TXN-IDFC-100)",
     "url": "/api/toll-import/lookup?txn_ref=TXN-IDFC-100&vendor=livq", "hk": "u1", "expect": 404},

    # Fallback
    {"n": 11, "d": "primary miss + fallback hit via source_key",
     "url": "/api/toll-import/lookup?txn_ref=FALLBACK-KEY-1", "hk": "u1", "expect": 200},
    {"n": 12, "d": "both miss → 404 repr 'NOPE'",
     "url": "/api/toll-import/lookup?txn_ref=NOPE", "hk": "u1", "expect": 404},

    # repr() formatting axis (CRITICAL)
    {"n": 13, "d": "404 repr with single quote (double-quote wrap)",
     "url": "/api/toll-import/lookup?txn_ref=it%27s", "hk": "u1", "expect": 404},
    {"n": 14, "d": "404 repr with double quote (single-quote wrap)",
     "url": "/api/toll-import/lookup?txn_ref=a%22b", "hk": "u1", "expect": 404},
    {"n": 15, "d": "404 repr with backslash",
     "url": "/api/toll-import/lookup?txn_ref=a%5Cb", "hk": "u1", "expect": 404},
    {"n": 16, "d": "404 repr with newline",
     "url": "/api/toll-import/lookup?txn_ref=a%0Ab", "hk": "u1", "expect": 404},
    {"n": 17, "d": "404 repr with space",
     "url": "/api/toll-import/lookup?txn_ref=a%20b%20c", "hk": "u1", "expect": 404},
    {"n": 18, "d": "404 repr with tab",
     "url": "/api/toll-import/lookup?txn_ref=a%09b", "hk": "u1", "expect": 404},

    # Isolation / tenant
    {"n": 19, "d": "wrong user u2 cannot see u1 TXN-IDFC-100 → 404",
     "url": "/api/toll-import/lookup?txn_ref=TXN-IDFC-100", "hk": "u2", "expect": 404},
    {"n": 20, "d": "u1 default co-a cannot see co-a-alt TXN-ALT-1 → 404",
     "url": "/api/toll-import/lookup?txn_ref=TXN-ALT-1", "hk": "u1", "expect": 404},
    {"n": 21, "d": "owned X-Company-Id co-a-alt → e-alt visible",
     "url": "/api/toll-import/lookup?txn_ref=TXN-ALT-1", "hk": "u1",
     "extra_hdr": {"X-Company-Id": "co-a-alt"}, "expect": 200},
    {"n": 22, "d": "unowned X-Company-Id co-b → fallback co-a → e-idfc-1 visible",
     "url": "/api/toll-import/lookup?txn_ref=TXN-IDFC-100", "hk": "u1",
     "extra_hdr": {"X-Company-Id": "co-b"}, "expect": 200},
]


async def run() -> int:
    print(f"[gate7k] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate7k] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate7k_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate7k_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 7k · LIVE PARITY MATRIX (Toll-import lookup)")
        print("=" * 72)
        for r in results:
