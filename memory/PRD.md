...
- [x] **Iter126b · Bucket-B Idempotency-Key middleware — 24 h replay + single-flight** (Feb 2026 — LOCKED ✅)
  ...
  - **Isolation**: composite `_id` includes both user_id and active company_id, so the same key from a different user OR a different active company can never replay another caller's response. Verified by test 3.
  - **Storage**: `db.idempotency_keys` collection with a 24 h TTL index on `created_at` (created idempotently at startup). Body stored base64, ≤ 5 MiB cap; larger responses drop the claim so a retry re-runs (the three A→B LR endpoints are already duplicate-safe on re-run via deterministic lr_number back-fill).
  - **5xx are NOT cached** — a transient 502/503 during a Save (the exact P0 scenario Iter126 was built for) leaves no cache row, so the client retry runs the handler again; the eventual success is then cached and every further retry replays it. 4xx responses ARE cached (spare the client from resubmitting a bad payload).
  - **Passthrough safety**: request has no header → passthrough. Key < 8 or > 128 chars → passthrough. Route not in Bucket-B whitelist → passthrough. Unauthenticated request → passthrough (endpoint issues its own 401 without polluting the cache).
  - **Locked Bucket-B whitelist** (see `/app/backend/idempotency.py :: BUCKET_B_PATTERNS`) — 34 patterns covering every create/payment/upload POST plus the three A→B LR moves (`regenerate-lr`, `bulk-regenerate-lr`, `bulk-all-copies-zip`). No Bucket-A or Bucket-C route is in this list.
  - **Middleware order** (outer → inner): `Idempotency → SaveHealth → CORS → route`. Replays bypass the route entirely (no duplicate mutation) while preserving observability for the ORIGINAL request (already logged the first time by save_health).
  - **Untouched surface**: auth (Iter106/106b/110), Iter120 regression guard, Iter121 loopback filter, Iter122 single-trip ZIP, Iter123 hero, Iter124 LR Register, Iter125 bulk ZIP, all freight / shortage / supplier / invoice / LR calculation code. Zero-diff to Bucket A retry (Iter126a) — this middleware sits in front of and independent of it.
  - **Tests** — `tests/test_iter126b_idempotency.py` (7/7 pass in 9.37 s): (1) same-key trip create replays + no duplicate row, (2) 5 concurrent same-key requests → 1 record, (3) same key + different active company does NOT replay, (4a) customer add-payment dedupes (advance_balance never doubles), (4b) supplier payment dedupes + ledger has one row, (5) 4xx cached vs. success replay behaviour, (6) Bucket A/C routes stay passthrough. Wired into `scripts/run_regression.sh`.
  - **What's next**: Iter126c (sessionStorage form-state preservation) is DEFERRED — do not implement without a fresh user go-ahead. Deploy Readiness Badge and LR Register Email Digest are also on hold.

- [x] **Iter126a · Write-Path Resilience Phase 1 — storage-startup fix + Axios Bucket-A retry** (Feb 2026 — LOCKED ✅)
  - **Root cause of the P0 "REFRESHING…" data-loss report**: transient 502/503/504s (and the pod-restart network gap) never triggered any client retry. A user hitting Save during a backend reload got a hard error and their form buffer was gone.
  - **Storage bug RCA (fix included here)**: `storage_client.init_storage()` is a **sync** function returning `str`. `server.py:900` was calling `await init_storage()` — that raised `TypeError: object str can't be used in 'await' expression` on every boot, swallowed as a warning, leaving `_storage_key` un-primed until the first real upload lazy-initialised it. Fix: `await loop.run_in_executor(None, init_storage)` inside the async startup event; `storage_client.py` stays untouched (all other callers already treat it as sync). Startup log now reads `Object storage initialized`.
  - **Endpoint Safety Matrix (LOCKED — do not reclassify without another user review)**:
    - **Bucket A — auto-retry (naturally idempotent)**: all `GET/HEAD/OPTIONS`; all `PUT/PATCH/DELETE`; and the whitelisted read-only POSTs: `POST /companies/{cid}/set-default`, `POST /auth/logout`, `POST /trips/lr/preview`, `POST /trips/bulk-invoice-preflight`, `POST /policy-changes/preview`, `POST /vehicles/bulk-import/preview`.
    - **Bucket B — needs Idempotency-Key (Iter126b — NOT YET BUILT)**: every create/payment/upload POST (create Trip / Company / Customer / Supplier / Driver / Vehicle / Invoice / Product / Party / Team / Template / Fuel / expenditure-type / saved-filter / driver-shortage-policy; all `/payments`, `/ledger*`, `/salary-masters`; every sub-resource create like `ship-sites`, `supplier-diesel`, `supplier-advance`, `field-override`; `duplicate` / `from-template` / `quick-repeat`; `files/upload`, `files/bulk-upload`, `company/logo`; `trips/import`, `vehicles/bulk-import`). **Plus the three user-mandated moves from A → B**: `POST /trips/{tid}/regenerate-lr`, `POST /trips/bulk-regenerate-lr`, `POST /trips/bulk-all-copies-zip` (they can back-fill `lr_number` and write audit rows, so retries must be duplicate-safe).
    - **Bucket C — never auto-retry**: bulk mutations (`bulk-delete`, `policy-changes/apply`, `policy-changes/{id}/revert`); communications (`invoices/{iid}/share`, `trips/{tid}/share-lr`, `customers/{cid}/share-statement`, `reports/supplier-statement/share`, `reminders/digest/run`); LLM cost calls (all `/ai/*` POSTs); auth handshakes (`/auth/session`, `/auth/demo-login`).
  - **Frontend change** — `frontend/src/api.js` gets one new response-error branch above the existing `/auth/me` handler: if `_isBucketA(config)` and `_isTransient(err)` (network error / 502 / 503 / 504), retry up to 2× with backoff [500 ms, 1500 ms]. Idempotent by design — no side-effect surface widened. Bucket B / C endpoints are explicitly not covered by this retry.
  - **Backend change** — `server.py:900` replaces `await init_storage()` with `await loop.run_in_executor(None, init_storage)`.
  - **Not touched** — no router logic, no auth flow, no LR / freight / shortage / supplier / invoice code, no Bucket B / C behaviour, no idempotency middleware yet, no sessionStorage form persistence (deferred to Iter126c, pending user go-ahead).
  - **Tests** — `tests/test_iter126a_storage_startup.py` (3 tests · 2 pass · 1 skipped when EMERGENT_LLM_KEY absent) guards: (1) `init_storage` stays sync, (2) it returns a real key when configured, (3) `server.py` never re-introduces `await init_storage()` in code (comment lines ignored). Wired into `scripts/run_regression.sh`. **Full Regression Guard: 348 passed, 1 skipped, 0 failed in 505.59 s** — GREEN.
  - **Rollout plan (user-approved)** — 126a shipped (this entry) → 126b Idempotency-Key + Mongo 24 h replay cache for Bucket B → 126c sessionStorage form-state preservation (only after 126b is green; do NOT store customer / invoice / payment / financial data in sessionStorage).

- [x] **Iter-audit-01 · Router classification decisions** (Feb 2026 — DOCUMENTATION ONLY, no code / DB / UI change)
  - **`ai.py` → LIVE / RETRO-APPROVED**: 11 endpoints (chat · voice → parse-trip/template/generic · insights · NL report). Actively used in production paths (`VoiceButton` inside InvoiceCreate/InvoiceView/TripTemplates, `VoiceTripButton` inside TripForm, `AIChatBubble`). Real usage confirmed (chat_sessions=6, chat_messages=17). **Retro-approved as-is** — do not modify, do not remove.
  - **`templates.py` → KEEP AS-IS**: 5 CRUD endpoints for Trip Templates + `TripTemplates.jsx` UI at `/trips/templates`. `trip_templates` collection currently empty. Remains on the P3 backlog "Trip Templates completion" — will be picked up separately. Do not hide, remove, or complete now.
  - **`driver_shortage_policies.py` → KEEP AS-IS (intentional capability)**: 6 endpoints, `DriverShortagePolicies.jsx` UI at `/drivers/shortage-policies`, 10,179 policy rows. **Intentional business capability** for future driver-wise shortage/recovery allowances (e.g. 50 KG, 100 KG). Exact policy values to be decided in an internal ERP review. Do not wire into trip calculation yet · do not assign any default value · do not hide · do not remove · do not touch the existing shortage engine (Iter98/102/107/111). Keep intact until business policy is finalised.
  - **Iter105 Policy Apply / Revert**: `policy_change_events = 1,039` rows — workflow proven via tests. Real-user UAT to be performed **later on a safe demo customer only** (never on MEGHA / Jalnidhi / any real customer). Preview is read-only-safe; Apply persists to trips and needs a throwaway `DEMO_ITER105_UAT` customer + 3-5 test trips + explicit teardown after Revert verification.
  - **What did NOT change**: No code, no database, no router mounting, no UI hiding, no default value assignment, no shortage engine touch, no Regression Guard reset. Only `/app/memory/PRD.md` note added for future context.
...
- [x] **Iter125 · Bulk All-Copies ZIP — selection OR filter mode** (Feb 2026 — APPROVED ✅ · LOCKED)
  ...
  - **Filename rule**: `LR_Bulk_All_Copies_<CODE>_<today>_<N>_trips.zip` for selection; `LR_Bulk_All_Copies_<CODE>_<YYYY-MM>_<N>_trips.zip` for month-aligned filter; `LR_Bulk_All_Copies_<CODE>_<start>to<end>_<N>_trips.zip` for custom range. Reuses Iter124 `_company_code()`.
  - **Audit**: One `audit_logs` row per bulk request, `action=lr_all_copies_zip` (same as Iter122 → Iter124 LR Register keeps counting them as `ZIP · N×`). Detail includes `included`, `skipped`, `skip_reasons`, `trip_ids`, `filename`, `period`, `generated_pdfs`.
  - **Frontend surfaces**:
    - `Trips.jsx` — new bulk toolbar button `bulk-all-copies-zip-btn` (`Archive` icon, indigo-700, shows selected count, disabled >200). Opens a confirmation modal (`bulk-all-copies-modal`) with: trips × 3 = N PDFs, estimated ZIP size, folder-per-LR note, skip-reason list, Cancel / Confirm & Download. After download → success view with Included / Skipped / reasons + `_manifest.txt` hint.
    - `Reports.jsx :: LRRegisterReport` — new button `lrreg-all-copies-zip` in the download row (next to Excel / PDF). Uses the current LR-Register filters as the payload. Same modal component pattern for confirmation + post-download summary.
    - `utils/pdfDownload.js` — new `downloadBulkAllCopiesZip(payload)` helper handling Bearer auth, filename parsing, header extraction, and blob error decoding.
  - **Tests** — `tests/test_iter125_bulk_all_copies.py` — **9/9 pass in 24.19 s**. Covers selection mode, filter mode (matches LR Register), month-aligned filename, custom-range filename, 200-cap, empty payload → 400, LR number back-fill without other mutation, manifest content, audit row.
  - **Not touched**: `_compute_trip`, supplier freight / shortage engine, invoice PDF, LR renderer (Iter115), `/trips/{tid}/lr` single-copy (Iter109 regenerate) and `/trips/{tid}/lr/all-copies` (Iter122) — both remain untouched. Iter105 policy, Iter106 auth, Iter121 loopback, Iter123 hero, Iter124 LR Register data loader — all zero-diff.

- [x] **Iter124c · LR Register live UI refinement — Indian dates + width + horizontal scroll** (Feb 2026 — APPROVED ✅ · LOCKED)
  - **User UAT verdict**: Indian date format correct, Date column single-line, long-text columns readable, horizontal scrolling works, sticky header aligned, column highlight moves correctly (including after scroll), filter chips + period clear, right-side columns fully accessible, XLSX/PDF respect filters.
  - **DO NOT MODIFY** the LR Register UI (`Reports.jsx :: LRRegisterReport`) without explicit user request. Endpoint (`GET /api/reports/lr-register{,.xlsx,.pdf}`) and its data loader also remain locked.
