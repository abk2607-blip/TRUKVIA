...
def build_debit_note_pdf(company, customer, invoice, note):
    ...
    if note.get("apply_gst", True) is False:
        gst_badge = Table([[Paragraph("<b>GST NOT APPLIED</b> · This Debit Note is issued without GST at the party's election. All statutory validations still apply.", small)]],
                          colWidths=[186*mm])
    ...
    final_label = "TOTAL DEBIT NOTE (excl. RCM GST)" if (is_rcm and apply_gst) else "TOTAL DEBIT NOTE"
    ...
    if note.get("rcm"):
        ...
        rcm_box = Table([[Paragraph(
            "<b>REVERSE CHARGE MECHANISM (RCM)</b> — GST on this Debit Note is to be paid by the recipient under Notification No. 08/2017. "
