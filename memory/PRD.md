...
- **Iter133 · L1.5 · Passbook CN/DN Row Detail** — ✅ VERIFIED (awaiting UAT lock) · 2026-08-30
  ...
  - **Tests (new)**: `test_iter133_l1_5_passbook_cdn_rows.py` — **6/6 passing**: (T1) issued CN row shape; (T2) issued DN positive-sign convention; (T3) cancelled notes excluded (draft exclusion covered by shared `status:"issued"` filter); (T4) `date_from`/`date_to` filter on `note_date`; (T5) customer isolation — CN on cust-X invisible in cust-Y passbook; (T6) `summary.credits_total`/`debits_total` unchanged + `txn_type=credit_note|debit_note|trip` filter correctness.
  - **Regression**: L1 25/25 · L2/L2b/L2c 25/25 · Iter132a 10/10 · Iter132b 10/10 (+1 known-flake asyncio loop-closed, passes in isolation) · Iter132c C1 39/40 (+1 known cross-tenant-dashboard flake — dashboard endpoint sees higher total than mongo ground-truth by ₹28k due to concurrent test writes; passes in isolation) · Iter132c C2/C2b 25/25 · Iter132c-agg-fix 4/4. Total isolated pass rate: 100%.
  ...
  - **Not shipped (still deferred)**: Slice C3 GSTR-1 Section 9B export, Slice C4 CN/DN Register report, Iter132c-agg-fix `to_list` in `ai.py`/`services.py`, Statement Email Delivery, Phase 2 Security Hardening, `AKB/26-27//26-27/0004` hygiene, Preview Uptime Chip, LR Register Email Digest, Trip Templates completion.
