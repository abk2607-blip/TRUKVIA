"""Phase 3 · Gate 6z · live Python↔Node parity harness for
VendorPayment list reads.

Covers:
  * GET /api/vendors/{vid}/payments

STRICT UAT-DATA PRESERVATION:
  Uses an isolated timestamped DB (`trukvia_gate6z_parity_<ts>`) that
...
Gate-6z dimensions bound in this harness:
  * SINGLE soft-delete predicate `is_deleted: {$ne: True}` (NO
    `is_reversed` predicate — see routers/vendors.py:171–179).
  * Projection strips BOTH `_id` and `user_id` (differs from Gate 6y
    drivers, which keeps `user_id`).
  * `activeCompanyId()` consumed (X-Company-Id changes rowset).
  * 5000 cap, date DESC ordering.
  * NO vendor-existence lookup → unknown / wrong-company / no-match
...
DB = f"trukvia_gate6z_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8206, 8207
...
def _vp(**kw: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
        "id": "vp-x", "user_id": "u1", "company_id": "co-a",
        "vendor_id": "ven-1",
        "date": D1, "amount": 2500.0, "type": "payment_out",
...
def fixtures() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        "vendor_payments": [
            _vp(id="p1", date=D1, ref_no="RN-1"),
            _vp(id="p2", date=D2, ref_no="RN-2"),
            _vp(id="p-del", date=D1, ref_no="RN-DEL", is_deleted=True),
            _vp(id="p-ven2", vendor_id="ven-2", date=D1, ref_no="RN-V2"),
            _vp(id="p-alt", company_id="co-a-alt", date=D1, ref_no="RN-ALT"),
            _vp(id="p-u2",  user_id="u2", company_id="co-b", date=D1, ref_no="RN-U2"),
        ],
    }
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6z_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6z_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    {"n":  1, "d": "list happy · u1 · co-a · ven-1 · [p1,p2] DESC (deleted excluded, alt-co hidden)",
     "url": "/api/vendors/ven-1/payments", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  2, "d": "empty · ven-empty (no payments) → 200 []",
     "url": "/api/vendors/ven-empty/payments", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  3, "d": "unknown vendor · no vendor-existence lookup → 200 []",
     "url": "/api/vendors/does-not-exist/payments", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  4, "d": "different vendor · ven-2 → [p-ven2]",
     "url": "/api/vendors/ven-2/payments", "hk": "u1", "expect": 200, "compare": "full"},
    {"n":  5, "d": "cross-user · u2 · ven-1 → [p-u2]",
     "url": "/api/vendors/ven-1/payments", "hk": "u2", "expect": 200, "compare": "full"},
    {"n":  6, "d": "owned X-Company-Id override · co-a-alt · ven-1 → [p-alt]",
     "url": "/api/vendors/ven-1/payments", "hk": "u1",
    ...
    {"n":  8, "d": "no auth → 401 Not authenticated",
     "url": "/api/vendors/ven-1/payments", "hk": "none", "expect": 401, "compare": "full"},
    {"n":  9, "d": "invalid bearer → 401 Invalid session",
     "url": "/api/vendors/ven-1/payments", "hk": "bad", "expect": 401, "compare": "full"},
    {"n": 10, "d": "expired bearer → 401 Session expired",
     "url": "/api/vendors/ven-1/payments", "hk": "expired", "expect": 401, "compare": "full"},
]
...
async def run() -> int:
    print(f"[gate6z] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate6z] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate6z_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate6z_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 6z · LIVE PARITY MATRIX (VendorPayments list)")
        print("=" * 72)
        for r in results:
