...
- [x] **Iter102 · Round-Trip KM Freight Method Fix + Multi-Trip Supplier Statement UAT** (Feb 2026)
  - **RCA (P0 issue)**: When freight mode was `fixed` + round-trip KM (`round_trip_kms × rate_per_km_per_ton`), the central engine multiplied by `tons` unconditionally — the customer's frozen `applied_freight_method` (Unloading Qty / Higher-of / Loading) was silently ignored on round-trip bills, breaking supplier statement parity.
  ...
  - **Fix (frontend)** — `TripForm.jsx` live freight preview mirrors the same resolver so the on-screen number stays paisa-accurate to the backend for all 4 methods, in both per-ton and round-trip modes.
  - **New regression guards** — `test_iter102_round_trip_freight_methods.py` locks the Round-Trip KM math for all four methods + snapshot immutability under later master edits. `test_iter102_multi_trip_supplier_uat.py` guards the full Trip → Invoice PDF → Supplier Statement JSON+PDF parity chain (4 customer trips + 4 supplier trips, mixed freight methods, halting, diesel, advance, shortage-deduction, net-payable — all must match to the paisa) + a `net_shortage` vs `full_after_limit` shortage-method separation test.
