"""Phase 3 · Gate 6l · live Python↔Node parity harness for Mechanic reads.
...
STRICT UAT-DATA PRESERVATION:
  Uses an isolated timestamped DB (`trukvia_gate6l_parity_<ts>`) that
...
DB = f"trukvia_gate6l_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8177, 8178
...
def _mech(**kw: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
        "id": "m-x", "user_id": "u1", "company_id": "co-a",
        "name": "Mechanic X", "mobile": "9999999999",
        "skill": "General", "address": "", "is_active": True,
        "created_at": now_iso(),
    }
    base.update(kw)
    return base
...
def fixtures() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        "mechanics": [
            _mech(id="m-alpha", name="Alpha Motors",   mobile="9111111111", skill="Engine",       is_active=True),
            _mech(id="m-beta",  name="Beta Garage",    mobile="9222222222", skill="Brakes",       is_active=True, extra_field="preserved"),
            _mech(id="m-gamma", name="Gamma Workshop", mobile="9333333333", skill="Transmission", is_active=False),
            _mech(id="m-delta", name="Delta Repair",   mobile="9444444444", skill="Bodywork",     is_active=True),
            _mech(id="m-alt", user_id="u1", company_id="co-a-alt",
                  name="Alt Mechanic", mobile="9555555555", skill="Electrical"),
            _mech(id="m-u2", user_id="u2", company_id="co-b",
                  name="U2 Mechanic", mobile="9666666666", skill="General"),
        ],
    }
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6l_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6l_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    {"n": 1,  "d": "list happy path",
     "url": "/api/mechanics", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 2,  "d": "active_only=true → filters inactive",
     "url": "/api/mechanics?active_only=true", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 3,  "d": "active_only=false → all",
     "url": "/api/mechanics?active_only=false", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 4,  "d": "active_only=YES (TRUE token)",
     "url": "/api/mechanics?active_only=YES", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 5,  "d": "active_only=0 (FALSE token)",
     "url": "/api/mechanics?active_only=0", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 6,  "d": "active_only=on (TRUE token)",
     "url": "/api/mechanics?active_only=on", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 7,  "d": "active_only=off (FALSE token)",
     "url": "/api/mechanics?active_only=off", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 8,  "d": "invalid boolean → 422",
     "url": "/api/mechanics?active_only=maybe", "hk": "owner", "expect": 422, "compare": "status_only"},
    {"n": 9,  "d": "q matches name (lowercase)",
     "url": "/api/mechanics?q=alpha", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 10, "d": "q matches name UPPERCASE",
     "url": "/api/mechanics?q=ALPHA", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 11, "d": "q matches mobile",
     "url": "/api/mechanics?q=9222222222", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 12, "d": "q matches SKILL (Transmission)",
     "url": "/api/mechanics?q=Transmission", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 13, "d": "q matches skill lowercase (brakes)",
     "url": "/api/mechanics?q=brakes", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 14, "d": "q partial substring on mobile",
     "url": "/api/mechanics?q=3333", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 15, "d": "q no match → empty array",
     "url": "/api/mechanics?q=zzznomatch", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 16, "d": "empty q → filter omitted",
     "url": "/api/mechanics?q=", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 17, "d": "active_only + q combined (skill)",
     "url": "/api/mechanics?active_only=true&q=Brakes", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 18, "d": "cross-user isolation (u2 view)",
     "url": "/api/mechanics", "hk": "u2", "expect": 200, "compare": "full"},
    {"n": 19, "d": "owned X-Company-Id override → alt rows",
     "url": "/api/mechanics", "hk": "owner",
     "extra_hdr": {"X-Company-Id": "co-a-alt"}, "expect": 200, "compare": "full"},
    {"n": 20, "d": "unowned X-Company-Id → default fallback",
     "url": "/api/mechanics", "hk": "owner",
     "extra_hdr": {"X-Company-Id": "co-b"}, "expect": 200, "compare": "full"},
    {"n": 21, "d": "list no auth → 401",
     "url": "/api/mechanics", "hk": "none", "expect": 401, "compare": "full"},
    {"n": 22, "d": "list invalid bearer → 401",
     "url": "/api/mechanics", "hk": "bad", "expect": 401, "compare": "full"},
    {"n": 23, "d": "list expired → 401",
     "url": "/api/mechanics", "hk": "expired", "expect": 401, "compare": "full"},
    {"n": 24, "d": "detail happy",
     "url": "/api/mechanics/m-alpha", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 25, "d": "detail extra fields preserved",
     "url": "/api/mechanics/m-beta", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 26, "d": "detail INACTIVE mechanic STILL RETURNED",
     "url": "/api/mechanics/m-gamma", "hk": "owner", "expect": 200, "compare": "full"},
    {"n": 27, "d": "detail missing → 404",
     "url": "/api/mechanics/does-not-exist", "hk": "owner", "expect": 404, "compare": "full"},
    {"n": 28, "d": "detail cross-user → 404",
     "url": "/api/mechanics/m-u2", "hk": "owner", "expect": 404, "compare": "full"},
    {"n": 29, "d": "detail cross-company → 404",
     "url": "/api/mechanics/m-alt", "hk": "owner", "expect": 404, "compare": "full"},
    {"n": 30, "d": "detail owned X-Company-Id override → alt row",
     "url": "/api/mechanics/m-alt", "hk": "owner",
     "extra_hdr": {"X-Company-Id": "co-a-alt"}, "expect": 200, "compare": "full"},
    {"n": 31, "d": "detail no auth → 401",
     "url": "/api/mechanics/m-alpha", "hk": "none", "expect": 401, "compare": "full"},
    {"n": 32, "d": "detail invalid bearer → 401",
     "url": "/api/mechanics/m-alpha", "hk": "bad", "expect": 401, "compare": "full"},
    {"n": 33, "d": "detail expired → 401",
     "url": "/api/mechanics/m-alpha", "hk": "expired", "expect": 401, "compare": "full"},
]
...
async def run() -> int:
    print(f"[gate6l] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate6l] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate6l_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate6l_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 6l · LIVE PARITY MATRIX (Mechanic reads)")
        print("=" * 72)
        for r in results:
