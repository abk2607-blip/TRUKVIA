...
def build_invoice_pdf(company: dict, customer: dict, invoice: dict, trips: list) -> bytes:
    ...
    header_tbl = Table(
        [[company_para, [title_para, Spacer(1, 4), rcm_para]]],
    ...
    # --- Trip line items ---
    header = ["#", "Date", "Vehicle No", "Load", "Route", "Tons", "Rate Mode", "Rate", "Freight (Rs)"]
    rows = [header]
    for idx, t in enumerate(trips, start=1):
        route = f"{t.get('from_location','')} → {t.get('to_location','')}"
        mode = "Per Ton" if t.get("freight_mode") == "per_ton" else "Fixed"
        rate = _fmt(t.get("rate_per_ton", 0)) if t.get("freight_mode") == "per_ton" else _fmt(t.get("fixed_amount", 0))
        rows.append([
            str(idx),
            t.get("date", ""),
