"""Phase 3 · Gate 6p · live Python↔Node parity harness for Driver
...
STRICT UAT-DATA PRESERVATION:
  Uses an isolated timestamped DB (`trukvia_gate6p_parity_<ts>`) that
...
Class-C stance:
  ...
  `db.driver_payment_corrections.find().sort()` with async iteration.
...
DB = f"trukvia_gate6p_parity_{int(time.time())}"
PY_PORT, NODE_PORT = 8185, 8186
...
def _dcorr(**kw: Any) -> dict[str, Any]:
    base: dict[str, Any] = {
...
def fixtures() -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        ...
        "driver_payment_corrections": [
            _dcorr(id="dc-d3", payment_id="pay-1", correction_index=3, correction_reason="third"),
            _dcorr(id="dc-d1", payment_id="pay-1", correction_index=1, correction_reason="first", extra_field="preserved"),
            _dcorr(id="dc-d2", payment_id="pay-1", correction_index=2, correction_reason="second"),
            _dcorr(id="dc-p2-d1", payment_id="pay-2", correction_index=1, correction_reason="p2-first"),
            _dcorr(id="dc-alt", company_id="co-a-alt", payment_id="pay-1", correction_index=1, correction_reason="alt-first"),
            _dcorr(id="dc-u2", user_id="u2", company_id="co-b", payment_id="pay-1", correction_index=1, correction_reason="u2-first"),
        ],
...
def start_py():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6p_py.log", "wb"), stderr=subprocess.STDOUT,
    )
...
def start_node():
    ...
    return subprocess.Popen(
        ...
        stdout=open("/tmp/gate6p_node.log", "wb"), stderr=subprocess.STDOUT,
    )
...
CASES: list[dict[str, Any]] = [
    {"n":  1, "d": "happy list · driver pay-1 · ASC by correction_index",
     "url": "/api/driver-payments/pay-1/corrections", "hk": "owner", "expect": 200, "compare": "full"},
    {"n":  2, "d": "unknown pid → 200 []",
     "url": "/api/driver-payments/no-such-pid/corrections", "hk": "owner", "expect": 200, "compare": "full"},
    {"n":  3, "d": "second pid (pay-2) returns its own driver corrections",
     "url": "/api/driver-payments/pay-2/corrections", "hk": "owner", "expect": 200, "compare": "full"},
    {"n":  4, "d": "cross-user isolation (u2 view — u2's own row)",
     "url": "/api/driver-payments/pay-1/corrections", "hk": "u2", "expect": 200, "compare": "full"},
    {"n":  5, "d": "owned X-Company-Id override → alt-company row",
     "url": "/api/driver-payments/pay-1/corrections", "hk": "owner",
    ...
    {"n":  7, "d": "no auth → 401 Not authenticated",
     "url": "/api/driver-payments/pay-1/corrections", "hk": "none", "expect": 401, "compare": "full"},
    {"n":  8, "d": "invalid bearer → 401 Invalid session",
     "url": "/api/driver-payments/pay-1/corrections", "hk": "bad", "expect": 401, "compare": "full"},
    {"n":  9, "d": "expired session → 401 Session expired",
     "url": "/api/driver-payments/pay-1/corrections", "hk": "expired", "expect": 401, "compare": "full"},
]
...
async def run() -> int:
    print(f"[gate6p] DB={DB} (isolated — UAT data untouched)")
    ...
    try:
        ...
        print(f"[gate6p] py={okp} node={okn}")
        if not (okp and okn):
            if not okp: print(open("/tmp/gate6p_py.log").read()[-2000:])
            if not okn: print(open("/tmp/gate6p_node.log").read()[-2000:])
            return 2
        ...
        print("\n" + "=" * 72)
        print("PHASE 3 · GATE 6p · LIVE PARITY MATRIX (Driver payment corrections)")
        print("=" * 72)
        for r in results:
