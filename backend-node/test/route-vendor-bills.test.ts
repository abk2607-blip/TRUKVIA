/**
 * TRUKVIA · Phase-3 · Gate-6v · VendorBill read-only shadow — Vitest.
 *
 * Covers:
 *   GET /api/vendor-bills?vendor_id=&vehicle_id=&repair_event_id=&trip_id=
 *   GET /api/vendor-bills/{bid}
 *
 * Fake-DB write-forbid pattern inlined (mirrors Gate 6t/6u). `$ne` matcher
...
interface State {
  ...
  vendor_bills: Record<string, unknown>[];
  writes: string[];
}
...
function fakeDb(state: State): Db {
  const rowsOf = (name: string): Record<string, unknown>[] =>
    ...
    : name === 'vendor_bills' ? state.vendor_bills
    : [];
...
// Descending bill_date so DESC sort yields b1 > b2 > b3 > b4.
const D1 = '2026-05-04';
const D2 = '2026-05-03';
const D3 = '2026-05-02';
const D4 = '2026-05-01';

function vb(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    id: 'vb-x', user_id: 'u1', company_id: 'co-a',
    vendor_id: 'ven-1', vehicle_id: 'veh-1',
    repair_event_id: 're-1', trip_id: 'trp-1',
    bill_date: D1, bill_amount: 2500, bill_number: 'INV-1001',
    vendor_name: 'Alpha Parts', narration: 'spare parts',
...
function makeState(): State {
  return {
    ...
    vendor_bills: [
      vb({ id: 'b1', bill_date: D1, is_deleted: false }),
      vb({ id: 'b2', bill_date: D2, is_deleted: false, vendor_id: 'ven-2', vehicle_id: 'veh-2', repair_event_id: 're-2', trip_id: 'trp-2' }),
      vb({ id: 'b3', bill_date: D3 }), // is_deleted MISSING → must be INCLUDED
      vb({ id: 'b4', bill_date: D4, is_deleted: null }), // null → included
      vb({ id: 'b-sd', bill_date: D1, is_deleted: true }), // soft-deleted → excluded
      vb({ id: 'b-alt', company_id: 'co-a-alt', bill_date: D1, is_deleted: false }),
      vb({ id: 'b-u2', user_id: 'u2', company_id: 'co-b', bill_date: D1, is_deleted: false }),
    ],
    writes: [],
  };
}
...
const CO_A_ACTIVE_IDS = ['b1', 'b2', 'b3', 'b4']; // sorted DESC by bill_date

describe('Gate-6v · VendorBill read-only shadow', () => {
  let state: State;
  ...
  // ── LIST ──────────────────────────────────────────────────────────
  it('1 happy list · u1 · co-a · four active rows DESC by bill_date', async () => {
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u1' });
  ...
  it('2 empty list · vehicle_id filter yields no rows → 200 []', async () => {
    const r = await get('/api/vendor-bills?vehicle_id=no-such', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
    expect(r.json()).toEqual([]);
  });

  it('3 cross-user isolation · u2 never sees u1 rows', async () => {
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u2' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['b-u2']);
  });

  it('4 cross-company isolation · co-a-alt row not visible in default co-a scope', async () => {
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).not.toContain('b-alt');
  });

  it('5 is_deleted:true excluded', async () => {
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).not.toContain('b-sd');
  });

  it('6 is_deleted:false included (b1, b2)', async () => {
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
  ...
  it('9 vendor_id filter · ven-2 → b2', async () => {
    const r = await get('/api/vendor-bills?vendor_id=ven-2', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['b2']);
  });

  it('10 vehicle_id filter · veh-2 → b2', async () => {
    const r = await get('/api/vendor-bills?vehicle_id=veh-2', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['b2']);
  });

  it('11 repair_event_id filter · re-2 → b2', async () => {
    const r = await get('/api/vendor-bills?repair_event_id=re-2', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['b2']);
  });

  it('12 trip_id filter · trp-2 → b2', async () => {
    const r = await get('/api/vendor-bills?trip_id=trp-2', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['b2']);
  });

  it('13 combined vendor_id + vehicle_id + repair_event_id + trip_id → b2', async () => {
    const r = await get('/api/vendor-bills?vendor_id=ven-2&vehicle_id=veh-2&repair_event_id=re-2&trip_id=trp-2', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['b2']);
  });

  it('14 empty-string filters omitted (truthy-gate) · returns all active co-a rows', async () => {
    const r = await get('/api/vendor-bills?vendor_id=&vehicle_id=&repair_event_id=&trip_id=', { authorization: 'Bearer tok-u1' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(CO_A_ACTIVE_IDS);
  });

  it('15 5000 cap · exactly 5000 returned when >5000 rows exist', async () => {
    state.vendor_bills.length = 0;
    for (let i = 0; i < 5001; i++) {
      state.vendor_bills.push(vb({
        id: 'z' + String(i).padStart(5, '0'),
        bill_date: `2026-06-${String((i % 28) + 1).padStart(2, '0')}`,
        is_deleted: false,
      }));
    }
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u1' });
  ...
  it('17 extra fields preserved (LIST) · bill_number/vendor_name/bill_amount', async () => {
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u1' });
    const first = (r.json() as Record<string, unknown>[])[0];
    expect(first['vendor_name']).toBe('Alpha Parts');
    expect(first['bill_number']).toBe('INV-1001');
    expect(first['bill_amount']).toBe(2500);
  });

  it('18 owned X-Company-Id override · co-a-alt → b-alt only', async () => {
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u1', 'x-company-id': 'co-a-alt' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(['b-alt']);
  });

  it('19 unowned X-Company-Id → falls back to default co-a', async () => {
    const r = await get('/api/vendor-bills', { authorization: 'Bearer tok-u1', 'x-company-id': 'co-b' });
    const ids = (r.json() as Record<string, unknown>[]).map((x) => x['id']);
    expect(ids).toEqual(CO_A_ACTIVE_IDS);
  });
  ...
  // ── DETAIL ────────────────────────────────────────────────────────
  it('21 detail happy · b1', async () => {
    const r = await get('/api/vendor-bills/b1', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(200);
  ...
  it('22 unknown bid → 404 exact literal', async () => {
    const r = await get('/api/vendor-bills/does-not-exist', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(404);
    expect(r.json()).toEqual({ detail: 'VendorBill not found' });
  });

  it('23 wrong-company bid → same 404 (b-u2 belongs to co-b)', async () => {
    const r = await get('/api/vendor-bills/b-u2', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(404);
    expect(r.json()).toEqual({ detail: 'VendorBill not found' });
  });

  it('24 soft-deleted bid → same 404 (b-sd is_deleted:true)', async () => {
    const r = await get('/api/vendor-bills/b-sd', { authorization: 'Bearer tok-u1' });
    expect(r.statusCode).toBe(404);
    expect(r.json()).toEqual({ detail: 'VendorBill not found' });
  });

  it('25 detail projection strips _id and user_id + extra fields preserved', async () => {
    const r = await get('/api/vendor-bills/b3', { authorization: 'Bearer tok-u1' });
    const body = r.json() as Record<string, unknown>;
    expect(body['id']).toBe('b3');
    expect(body['vendor_name']).toBe('Alpha Parts');
    expect(body['bill_amount']).toBe(2500);
    expect(body).not.toHaveProperty('_id');
    expect(body).not.toHaveProperty('user_id');
  });
  ...
  it('32 zero Node writes across every exercised branch', async () => {
    await get('/api/vendor-bills');
    await get('/api/vendor-bills', { authorization: 'Bearer nope' });
    await get('/api/vendor-bills', { authorization: 'Bearer tok-expired' });
    await get('/api/vendor-bills', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills?vendor_id=ven-2', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills?vehicle_id=veh-2', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills?repair_event_id=re-2', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills?trip_id=trp-2', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills?vendor_id=ven-2&vehicle_id=veh-2&repair_event_id=re-2&trip_id=trp-2', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills', { authorization: 'Bearer tok-u1', 'x-company-id': 'co-a-alt' });
    await get('/api/vendor-bills', { authorization: 'Bearer tok-u1', 'x-company-id': 'co-b' });
    await get('/api/vendor-bills/b1', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills/does-not-exist', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills/b-u2', { authorization: 'Bearer tok-u1' });
    await get('/api/vendor-bills/b-sd', { authorization: 'Bearer tok-u1' });
